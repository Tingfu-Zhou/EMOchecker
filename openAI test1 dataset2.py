...
utterances = texts_df['text_corrected'][:max_experiments]  # 只读取前 100 行
sentiments = texts_df['overall_sentiment'][:max_experiments]  # 获取前 100 行对应的情感标签
...
for file_id, utterance, sentiment in zip(file_ids, utterances, sentiments):
    ...
    run = client.beta.threads.runs.create_and_poll(
      thread_id=thread.id,
      assistant_id='asst_uQpSjkITLjA0Z6UEt2jrpMRw',  # 请替换为实际的 assistant ID
      instructions="Note: The case I will send you next is to test the prompt (Assistant). Classify emotions into the following categories ['very_positive' 'positive' 'neutral' 'negative' 'very_negative']. You only need to classify emotions and output the corresponding categories. You don't need to answer anything else. For example, If you judge that the character in the dialogue is happy based on the above method, you only need to output 'very_positive'."
    )
